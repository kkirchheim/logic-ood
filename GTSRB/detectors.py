...
class SemanticDetector(Detector):
    ...
    def predict(self, x):
        ...
        if self.sign_net:
            ...
            scores = (shapes_detected == shapes_expected) & (colors_expected == colors_detected) & (sign_expected == sign_detected) 
        else:
...
class KBDetector(Detector):
    ...
    def predict(self, x) -> torch.tensor:
        ...
        scores = torch.stack([labels_conf, shapes_conf, colors_conf, signs_conf], dim=1).mean(dim=1).cpu()
